...
import (
	...
	"github.com/mazzegi/adventofcode/adventofcode_2019/wire"
)

func main() {
	//in := inputTest2
	in := input
	p1, err := wire.ParsePath(in.RawPath1)
	if err != nil {
		panic(err)
	}
	p2, err := wire.ParsePath(in.RawPath2)
	if err != nil {
		panic(err)
	}
	fmt.Printf("path-1: %s\n", p1.String())
	...
	is := p1.Intersections(p2)
	fmt.Printf("intersections: %v\n", is)
	if len(is) == 0 {
		os.Exit(0)
	}

	minDist := is[0].ManhattanDist()
	for _, pt := range is {
		if pt.ManhattanDist() < minDist {
			minDist = pt.ManhattanDist()
		}
	}
	fmt.Printf("the distance is: %d\n", minDist)
	//
	p1.BestIntersection(p2)
}
...
var inputTest0 = Input{
	RawPath1:  "R8,U5,L5,D3",
	RawPath2:  "U7,R6,D4,L4",
	ExpResult: 6,
}

var inputTest1 = Input{
	RawPath1:  "R75,D30,R83,U83,L12,D49,R71,U7,L72",
	RawPath2:  "U62,R66,U55,R34,D71,R55,D58,R83",
	ExpResult: 159,
}

var inputTest2 = Input{
	RawPath1:  "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51",
	RawPath2:  "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7",
	ExpResult: 135,
}
