...
func setupTest() *burrow {
	...
	b.init(
		...
		&amphipod{1, atypA, location{idSideRoomC, 1}},
		&amphipod{2, atypA, location{idSideRoomD, 0}},
		&amphipod{3, atypA, location{idSideRoomD, 2}},

		&amphipod{0, atypB, location{idSideRoomA, 3}},
		&amphipod{1, atypB, location{idSideRoomB, 1}},
		&amphipod{2, atypB, location{idSideRoomC, 2}},
		&amphipod{3, atypB, location{idSideRoomC, 3}},

		&amphipod{0, atypC, location{idSideRoomB, 2}},
		&amphipod{1, atypC, location{idSideRoomB, 3}},
		&amphipod{2, atypC, location{idSideRoomC, 0}},
		&amphipod{3, atypC, location{idSideRoomD, 1}},

		&amphipod{0, atypD, location{idSideRoomA, 1}},
		&amphipod{1, atypD, location{idSideRoomA, 2}},
		&amphipod{2, atypD, location{idSideRoomB, 0}},
		&amphipod{3, atypD, location{idSideRoomD, 3}},
	)
	return b
}
...
func (b *burrow) possibleMovesForAmphipodInHallway(a *amphipod) []move {
	...
	} else if dstRoom.positions[1] == nil {
		steps += 3
		energy := steps * stepEnergy[a.typ]
		mvs = append(mvs, move{
			a,
	...
	} else if dstRoom.positions[2] == nil {
		steps += 2
		energy := steps * stepEnergy[a.typ]
		mvs = append(mvs, move{
			a,
			location{dstRoom.roomID, 2},
			energy,
		})
	} else {
		steps += 1
...
func (b *burrow) possibleMovesForAmphipodInSideRoom(a *amphipod, sr *sideRoom) []move {
	if a.loc.pos < 3 {
		for pos := a.loc.pos + 1; pos <= 3; pos++ {
			if sr.positions[pos] != nil {
				// exit is occupied
				return []move{}
			}
		}
	...
	if sr.typ == a.typ {
		...
		if a.loc.pos == 0 {
			...
			if sr.positions[0] != nil && sr.positions[0].typ == a.typ {
				return []move{}
			}
		} else if a.loc.pos == 2 {
			if sr.positions[0] != nil && sr.positions[0].typ == a.typ &&
				sr.positions[1] != nil && sr.positions[1].typ == a.typ {
				return []move{}
			}
		} else if a.loc.pos == 3 {
			if sr.positions[0] != nil && sr.positions[0].typ == a.typ &&
				sr.positions[1] != nil && sr.positions[1].typ == a.typ &&
				sr.positions[2] != nil && sr.positions[2].typ == a.typ {
				return []move{}
			}
		}
		// else {
...
func setup() *burrow {
	...
	b.init(
		...
		&amphipod{1, atypB, location{idSideRoomB, 1}},
		&amphipod{2, atypB, location{idSideRoomC, 2}},
		&amphipod{3, atypB, location{idSideRoomD, 3}},

		&amphipod{0, atypC, location{idSideRoomB, 2}},
		&amphipod{1, atypC, location{idSideRoomB, 3}},
		&amphipod{2, atypC, location{idSideRoomC, 3}},
		&amphipod{3, atypC, location{idSideRoomD, 1}},

		&amphipod{0, atypD, location{idSideRoomA, 0}},
		&amphipod{1, atypD, location{idSideRoomA, 1}},
		&amphipod{2, atypD, location{idSideRoomA, 2}},
		&amphipod{3, atypD, location{idSideRoomB, 0}},
	)
	return b
}
