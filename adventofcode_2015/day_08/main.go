...
func main() {
	...
	var totalDecodedMemChars int
	var totalEncodedMemChars int
	...
	for scanner.Scan() {
		...
		totalCodeChars += len(s)

		decs, err := decode(s[1 : len(s)-1])
		if err != nil {
			panic(fmt.Sprintf("in %q: %v", s, err))
		}
		totalDecodedMemChars += len(decs)

		encs := encode(s)
		totalEncodedMemChars += len(encs)

	}
	fmt.Printf("decode: %d code chars, %d mem chars, diff: %d\n", totalCodeChars, totalDecodedMemChars, totalCodeChars-totalDecodedMemChars)
	fmt.Printf("encode: %d code chars, %d mem chars, diff: %d\n", totalCodeChars, totalEncodedMemChars, totalEncodedMemChars-totalCodeChars)
}
...
func decode(s string) (string, error) {
	...
	for e, rep := range replaceHex {
		s = strings.ReplaceAll(s, e, rep)
	}
	s = strings.ReplaceAll(s, `$`, `\`)
	return s, nil
}
