...
func main() {
	...
	fmt.Printf("scanned %d actions\n", len(actions))

	g := newGrid(1000)
	for _, a := range actions {
		g.update(a.from, a.to, func(in int) int {
			switch a.what {
			case turnOn:
				return in + 1
			case turnOff:
...
func parseAction(s string) (action, error) {
	var what string
	switch {
	case strings.HasPrefix(s, turnOn):
		what = turnOn
		s = strings.TrimPrefix(s, turnOn)
	case strings.HasPrefix(s, turnOff):
		what = turnOff
		s = strings.TrimPrefix(s, turnOff)
	case strings.HasPrefix(s, toggle):
		what = toggle
		s = strings.TrimPrefix(s, toggle)
	default:
		return action{}, errors.Errorf("unknown action %q", s)
	}
	s = strings.Trim(s, " ")
	a := action{
		what: what,
	}
	_, err := fmt.Sscanf(s, "%d,%d through %d,%d", &a.from.x, &a.from.y, &a.to.x, &a.to.y)
	if err != nil {
		return action{}, err
	}
	return a, nil
...
func newGrid(dim int) *grid {
	...
	for ri := 0; ri < dim; ri++ {
		g.rows[ri] = make([]int, dim)
	}
	return g
}

func (g *grid) valueSum() int {
	var sum int
	for ri := 0; ri < g.dim; ri++ {
		for ci := 0; ci < g.dim; ci++ {
			sum += g.rows[ri][ci]
		}
	}
	return sum
...
func (g *grid) update(from, to pos, updateFunc func(curr int) int) {
	for ri := from.y; ri <= to.y; ri++ {
		for ci := from.x; ci <= to.x; ci++ {
			ns := updateFunc(g.rows[ri][ci])
			g.rows[ri][ci] = ns
		}
	}
}
