...
func main() {
	...
	fmt.Printf("scanned %d sues\n", len(sues))

	for _, sue := range sues {
		if sue.matches(machineOutput) {
			fmt.Printf("Sue %d matches\n", sue.ID)
		}
	}
}

func sueTrimmed(s string) string {
	return strings.Trim(s, ":,")
}

func ParseSue(s string) (Sue, error) {
	fs := strings.Fields(s)
	if len(fs) < 2 || len(fs)%2 != 0 {
		return Sue{}, errors.Errorf("invalid field count %d", len(fs))
	}

	id, err := strconv.ParseInt(sueTrimmed(fs[1]), 10, 16)
	if err != nil {
		return Sue{}, err
	}
	sue := Sue{
	...
	for i := 2; i < len(fs)-1; i += 2 {
		name := sueTrimmed(fs[i])
		num, err := strconv.ParseInt(sueTrimmed(fs[i+1]), 10, 16)
		if err != nil {
			return Sue{}, err
		}
		sue.Items[name] += int(num)
	}
	return sue, nil
}
...
func (s Sue) matchesEventually(oitems map[string]int) bool {
	for name, cnt := range s.Items {
		if ocnt, ok := oitems[name]; !ok || cnt != ocnt {
			return false
		}
	}
	return true
...
func (s Sue) matches(oitems map[string]int) bool {
	for name, cnt := range s.Items {
		if !matchesMachine(name, cnt) {
			return false
		}
	}
	return true
...
func matchesMachine(name string, cnt int) bool {
	mcnt, ok := machineOutput[name]
	if !ok {
		return false
	}
	switch name {
...
var machineOutput = map[string]int{
	"children":    3,
	"cats":        7,
	"samoyeds":    2,
	"pomeranians": 3,
