...
func main() {
	...
	for scanner.Scan() {
		...
		rule, err := DecodeRule(s)
		if err != nil {
			panic(err)
		}

		if _, contains := rules[rule.Color]; contains {
			panic(fmt.Sprintf("double rule %q", rule.Color))
		}
	...
	var cnt int
	for _, r := range rules {
		if r.Contains(mcr, rules) {
			cnt++
		}
	}
	fmt.Printf("%d bag colors eventually contains at least 1 %q\n", cnt, mcr)

	sgRule, ok := rules[mcr]
	if !ok {
		panic("no shiny gold rule")
	}
	tot := sgRule.ContainsTotal(rules)
	fmt.Printf("%q contains a total of %d other bags\n", mcr, tot)
}
...
func (r Rule) Contains(cr string, rules map[string]Rule) bool {
	for _, c := range r.Contents {
		if c.Color == cr {
			return true
		}
		if crule, ok := rules[c.Color]; ok {
			contains := crule.Contains(cr, rules)
			if contains {
				return true
			}
		}
...
func (r Rule) ContainsTotal(rules map[string]Rule) int {
	var tot int
	for _, c := range r.Contents {
		crule, ok := rules[c.Color]
		if !ok {
			panic(fmt.Sprintf("no %q rule", c.Color))
		}
		ctot := c.Amount * (1 + crule.ContainsTotal(rules))
		tot += ctot
	}
	fmt.Printf("tot for %q: %d\n", r.Color, tot)
	return tot
}
...
func (r Rule) String() string {
	var csl []string
	for _, c := range r.Contents {
		csl = append(csl, c.String())
	}
	return fmt.Sprintf("%q: %s", r.Color, strings.Join(csl, ", "))
}

func (c Content) String() string {
	return fmt.Sprintf("%d:%q", c.Amount, c.Color)
}
...
func DecodeRule(s string) (Rule, error) {
	...
	n := strings.Index(s, p)
	if n < 0 {
		return Rule{}, errors.Errorf("found no %q phrase", p)
	}
	cr := s[:n]
	s = strings.Trim(s[n+len(p):], " .")
	rule := Rule{
		Color:    cr,
		Contents: map[string]Content{},
	}
	...
	sl := strings.Split(s, ",")
	for _, cs := range sl {
		c, err := ParseContent(cs)
		if err != nil {
			return Rule{}, errors.Wrapf(err, "parse-content %q", cs)
		}
		if _, contains := rule.Contents[c.Color]; contains {
			return Rule{}, errors.Errorf("there's already content %q", c.Color)
		}
		rule.Contents[c.Color] = c
	}
...
func ParseContent(s string) (Content, error) {
	...
	sl := strings.Split(s, " ")
	if len(sl) != 4 {
		return Content{}, errors.Errorf("invalid content %q", s)
	}
	n, err := strconv.ParseInt(sl[0], 10, 64)
	if err != nil {
		return Content{}, errors.Wrapf(err, "parse-int %s", sl[0])
	}
	cr := strings.Trim(strings.Join(sl[1:3], " "), " ")
	return Content{
		Color:  cr,
		Amount: int(n),
	}, nil
