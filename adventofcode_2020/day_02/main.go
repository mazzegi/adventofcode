...
func main() {
	...
	for scanner.Scan() {
		...
		pol, pwd, err := ParsePolicyAndPassword(s)
		if err != nil {
			panic(err)
		}
		// if pol.MatchPwdMinMax(pwd) {
		// 	validCnt++
		// }
		match := pol.MatchPwdPos(pwd)
		if match {
			validCnt++
		}
		fmt.Printf("%q, %q -> %t\n", pol, pwd, match)
	}
	fmt.Printf("there are %d valid passwords in input\n", validCnt)
}

func ParsePolicyAndPassword(s string) (Policy, string, error) {
	sl := strings.Split(s, ":")
	if len(sl) != 2 {
		return Policy{}, "", errors.Errorf("invalid format %q - want <policy:pwd>", s)
	}
	pol, err := ParsePolicy(strings.Trim(sl[0], " "))
	if err != nil {
		return Policy{}, "", errors.Wrapf(err, "parse policy %q", sl[0])
	}
	return pol, strings.Trim(sl[1], " "), nil
}

func ParsePolicy(s string) (Policy, error) {
	var p Policy
	var char string
	n, err := fmt.Sscanf(s, "%d-%d %s", &p.P1, &p.P2, &char)
	if err != nil {
		return Policy{}, err
	}
	if n != 3 {
		return Policy{}, errors.Errorf("invalid scanned items count. have %d, want 3")
	}
	if p.P1 > p.P2 {
		return Policy{}, errors.Errorf("min (%d) may not be greater than max (%d)", p.P1, p.P2)
	}
	if len(char) != 1 {
		return Policy{}, errors.Errorf("invalid char %q", p.Char)
	}
	p.Char = rune(char[0])
	return p, nil
}
...
func (p Policy) String() string {
	return fmt.Sprintf("%d-%d %s", p.P1, p.P2, string(p.Char))
}

func (p Policy) MatchPwdMinMax(pwd string) bool {
	var cnt int
	for _, r := range pwd {
		if r == p.Char {
			cnt++
	...
	if cnt < p.P1 || cnt > p.P2 {
		return false
	}
	return true
}

func (p Policy) MatchPwdPos(pwd string) bool {
	matchIdx := func(s string, idx int, char rune) bool {
		if idx < 0 || idx >= len(s) {
			return false
		}
		return rune(s[idx]) == char
	}

	m1 := matchIdx(pwd, p.P1-1, p.Char)
	m2 := matchIdx(pwd, p.P2-1, p.Char)
