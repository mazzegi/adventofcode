...
func main() {
	...
	//buf := bytes.NewBufferString(input4Valid)
	scanner := bufio.NewScanner(buf)
	curr := []string{}
	ps := []passport.Passport{}
	...
	flush := func() {
		...
		p, err := passport.ParseStrings(curr)
		if err != nil {
			panic(err)
		}
		ps = append(ps, p)
	...
	fmt.Printf("got %d passports of which %d are valid\n", len(ps), validPassports)
